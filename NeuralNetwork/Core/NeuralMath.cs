...
namespace NeuralNetwork.Core
{
    static class NeuralMath
    {
        ...
        public static double[,] Sigmoid(double[,] z)
        {
            ...
            {
                output[0, i] = 1.0 / (1.0 + Math.Pow(Math.E, -z[0, i]));
            }
        ...
        public static double[,] SigmoidPrime(double[,] z)
        {
            ...
            for (int i = 0; i < z.GetLength(1); i++)
            {
                output[0, i] = 1 - (1.0 / (1.0 + Math.Pow(Math.E, -z[0, i])));
            }

            return DotMatrix(Sigmoid(z), output);
        }

        public static double[,] CostDerivative(double[,] outputActivations, int y)
        {
            double[,] output = new double[outputActivations.GetLength(0), outputActivations.GetLength(1)];

            for (int i = 0; i < outputActivations.GetLength(1); i++)
            {
                output[1, i] = outputActivations[1, i] - y;
            }
